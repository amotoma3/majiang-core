...
function get_post_hupai(shoupai, rongpai, baopai, fubaopai) {
    ...
    let n_baopai = new_shoupai._bingpai.bei;
    for (let p of baopai) {
...
function get_defen(fu, hupai, rongpai, param) {
    ...
    if (hupai[0].fanshu[0] == '*') {
        ...
        if (h) {
            baojia2 = (menfeng + { '+': 1, '=': 2, '-': 3}[h.baojia]) % 4; // TODO: 三麻のパオ責任者の挙動を確認する
    ...
    if (rongpai || base == 0) {
        ...
        if (menfeng == 0) {
            defen = zhuangjia * (param.rule['三麻'] ? 2 : 3);
            for (let l = 0; l < (param.rule['三麻'] ? 3 : 4); l++) {
                if (l == menfeng)
            ...
            defen = zhuangjia + sanjia * (param.rule['三麻'] ? 1 : 2);
            for (let l = 0; l < (param.rule['三麻'] ? 3 : 4); l++) {
                if (l == menfeng)
                        fenpei[l] += defen     + chang * 300 + lizhi * 1000;
                else if (l == 0)
